...
// Create Recipe Here :)
export async function createRecipe(userId, { title, ingredientAmounts = [], instructions, tags }) {
  const cleaned = ingredientAmounts
    ...
    .filter(
      (ia) =>
        ia.ingredient &&
        Number.isFinite(ia.amountQuantity) &&
        ia.amountUnit
    )
...
export async function updateRecipe(userId, recipeId, { title, ingredientAmounts, instructions, tags }) {
const cleaned = ingredientAmounts
    .map((ia) => ({
      ingredient: String(ia.ingredient ?? '').trim(),
      amountQuantity: Number(ia.amountQuantity),
